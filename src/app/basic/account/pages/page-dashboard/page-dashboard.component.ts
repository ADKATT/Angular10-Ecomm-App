...
export class PageDashboardComponent implements OnInit, OnDestroy {
	...
	*/
	ngOnInit(): void {
		this.userService.getDefaultAddress().pipe(takeUntil(this.destroy$)).subscribe(x => this.address = x);
		this.userService.getOrdersList({limit: 3}).pipe(takeUntil(this.destroy$)).subscribe(x => this.orders = x.items);
