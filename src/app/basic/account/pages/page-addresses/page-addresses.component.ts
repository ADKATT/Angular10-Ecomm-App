...
export class PageAddressesComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.userService.getAddresses().pipe(takeUntil(this.destroy$)).subscribe(x => this.addresses = x);
    }
    ...
    remove(address: Address): void {
        ...
        this.userService.delAddress(address.id).pipe(
            mergeMap(() => this.userService.getAddresses()),
            finalize(() => {
